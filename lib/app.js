...
app.get('/recipes/new',
  ...
  function (req, res, next) {
    ...
    knex('Recipes').select('tags')
    .then(function (results) {
      results.map(tags => {
        tags.tags.map(tag => availableTags.add(tag));
      });
      availableTags.delete('');
      res.render('recipes/_form.jade', { recipe: { ingredients: [] }, user: req.user, tags: Array.from(availableTags) });
    });
  });
...
app.get('/recipes/:id/edit',
  ...
  function (req, res, next) {
    ...
    .then(function (results) {
      res.render('recipes/_form.jade', { recipe: _.first(results), user: req.user, tags: [] });
    });
  });
