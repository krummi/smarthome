...
app.get('/budget', function (req, res, next) {
  ...
  let plannedExpensePerDay = 200000 / moment.utc().daysInMonth();
  let plannedExpenses = _.map(_.range(1, dayOfMonth + 1), function (i) {
    return { day: i, amount: plannedExpensePerDay };
  });
  console.log(sumByDay);
  console.log('total:', _.sum(_.values(sumByDay)));
  res.render('budget/index', { expensesByDay: sumByDay, plannedExpenses: plannedExpenses });
});
